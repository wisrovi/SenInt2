    ...
    def evaluarDatos(self, dato, conteo, banderaAlarma, umbral, tolerancia, etiqueta = 'T'):
        if umbral <= dato:
            ...
            if banderaAlarma == False:
                self.log.LlenarLogAuditoria("Borrando archivos en alarma ON")
                banderaAlarma = True
                if os.path.exists("0"):
                    os.system("rm 0")
                if os.path.exists("1"):
            ...
            if os.path.exists("0") == False:                
                ...
                self.log.LlenarLogAuditoria("Enviando datos al servidor, sistema alarmado")
                self.servidor.sendGet("?"+ etiqueta + "=" + str(dato))
                time.sleep(30)
            self.log.LlenarLogAuditoria("¡Se calentó el chuzo!")        
            ...
            if banderaAlarma == True:
                ...
                if umbralReducido <= dato:   
                    ...
                    self.log.LlenarLogAuditoria("Todo bien normalizando alarma")
                    banderaAlarma = False
            else:
                conteo = conteo + 1
                if conteo == 1:
                    self.log.LlenarLogAuditoria("Enviando datos al servidor, sistema normal")
                    self.servidor.sendGet("?"+ etiqueta + "=" + str(dato))
                ...
                self.log.LlenarLogAuditoria(datoImprimir)
                banderaAlarma = False
